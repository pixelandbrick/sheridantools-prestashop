...
include_once('htmlbanners7.php');

$hero_slider = new HtmlBanners7();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides')) {
    ...
    foreach ($slides as $position => $id_slide) {
        $res = Db::getInstance()->execute(
            '
			UPDATE `'._DB_PREFIX_.'htmlbanners7_slides` SET `position` = '.(int)$position.'
			WHERE `id_htmlbanners7_slides` = '.(int)$id_slide
        );
    }
