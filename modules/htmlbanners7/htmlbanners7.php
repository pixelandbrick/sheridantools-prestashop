...
include_once(_PS_MODULE_DIR_.'htmlbanners7/HtmlBanner7.php');

class HtmlBanners7 extends Module implements WidgetInterface
{
    protected $_html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('Html in after footer');
        $this->description = $this->l('Adds a html code to after footer');
        $this->ps_versions_compliancy = array('min' => '1.7.1.0', 'max' => _PS_VERSION_);

        $this->templateFile = 'module:htmlbanners7/views/templates/hook/hook.tpl';
    }
    ...
    protected function installSamples()
    {
        ...
        $slide = new HtmlBanner7();
        $slide->position = 1;
        $slide->active = 1;
        foreach ($languages as $language) {
            $slide->title[$language['id_lang']] = 'Copyright';
            $slide->description[$language['id_lang']] = '<p>© 2019 AutoParts. All rights reserved.</p>';
            $slide->customclass[$language['id_lang']] = 'copyright col-sm-12 col-md-4';
        }
        $slide->add();
        ...
        $slide->active = 1;
        foreach ($languages as $language) {
            $slide->title[$language['id_lang']] = 'Payment logos in footer';
            ...
            $slide->customclass[$language['id_lang']] = 'payment-logos col-sm-6 col-md-4';
        }
        $slide->add();
    }
    ...
    protected function createTables()
    {
        ...
        $res = (bool)Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'htmlbanners7` (
                `id_htmlbanners7_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
                `id_shop` int(10) unsigned NOT NULL,
                PRIMARY KEY (`id_htmlbanners7_slides`, `id_shop`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');

        /* Slides configuration */
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'htmlbanners7_slides` (
              `id_htmlbanners7_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
              `position` int(10) unsigned NOT NULL DEFAULT \'0\',
              `active` tinyint(1) unsigned NOT NULL DEFAULT \'0\',
              PRIMARY KEY (`id_htmlbanners7_slides`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');

        /* Slides lang configuration */
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'htmlbanners7_slides_lang` (
              `id_htmlbanners7_slides` int(10) unsigned NOT NULL,
              ...
              PRIMARY KEY (`id_htmlbanners7_slides`,`id_lang`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');
    ...
    protected function deleteTables()
    {
        ...
        foreach ($slides as $slide) {
            $to_del = new HtmlBanner7($slide['id_slide']);
            $to_del->delete();
        }

        return Db::getInstance()->execute('
            DROP TABLE IF EXISTS `'._DB_PREFIX_.'htmlbanners7`, `'._DB_PREFIX_.'htmlbanners7_slides`, `'._DB_PREFIX_.'htmlbanners7_slides_lang`;
        ');
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitSlide') || Tools::isSubmit('delete_id_slide') ||
            ...
            if ($mode == 'add') {
                ...
                $associated_shop_ids = HtmlBanner7::getAssociatedIdsShop((int)Tools::getValue('id_slide'));
    ...
    protected function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            $slide = new HtmlBanner7((int)Tools::getValue('id_slide'));
            if ($slide->active == 0) {
                $slide->active = 1;
            } else {
                $slide->active = 0;
            ...
            $slide = new HtmlBanner7((int)Tools::getValue('delete_id_slide'));
            $res = $slide->delete();
            $this->clearCache();
            if (!$res) {
    ...
    public function hookActionShopDataDuplication($params)
    {
        Db::getInstance()->execute(
            '
            INSERT IGNORE INTO '._DB_PREFIX_.'htmlbanners7 (id_htmlbanners7_slides, id_shop)
            SELECT id_htmlbanners7_slides, '.(int)$params['new_id_shop'].'
            FROM '._DB_PREFIX_.'htmlbanners7
    ...
    public function getNextPosition()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow(
            ...
            FROM `'._DB_PREFIX_.'htmlbanners7_slides` hss, `'._DB_PREFIX_.'htmlbanners7` hs
            WHERE hss.`id_htmlbanners7_slides` = hs.`id_htmlbanners7_slides` AND hs.`id_shop` = '.(int)$this->context->shop->id
        );
    ...
    public function getSlides($active = null)
    {
        ...
        $slides = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            '
            SELECT hs.`id_htmlbanners7_slides` as id_slide, hss.`position`, hss.`active`, hssl.`title`,
            hssl.`url`, hssl.`legend`, hssl.`customclass`, hssl.`description`, hssl.`image`
            FROM '._DB_PREFIX_.'htmlbanners7 hs
            LEFT JOIN '._DB_PREFIX_.'htmlbanners7_slides hss ON (hs.id_htmlbanners7_slides = hss.id_htmlbanners7_slides)
            LEFT JOIN '._DB_PREFIX_.'htmlbanners7_slides_lang hssl ON (hss.id_htmlbanners7_slides = hssl.id_htmlbanners7_slides)
        ...
        foreach ($slides as &$slide) {
            $slide['image_url'] = $this->context->link->getMediaLink(_MODULE_DIR_.'htmlbanners7/views/img/upload/'.$slide['image']);
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            ...
            LEFT JOIN '._DB_PREFIX_.'htmlbanners7_slides hss ON (hs.id_htmlbanners7_slides = hss.id_htmlbanners7_slides)
            LEFT JOIN '._DB_PREFIX_.'htmlbanners7_slides_lang hssl ON (hss.id_htmlbanners7_slides = hssl.id_htmlbanners7_slides)
            WHERE hs.`id_htmlbanners7_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
            ($active ? ' AND hss.`active` = 1' : ' ')
        );
    ...
    {
        $req = 'SELECT hs.`id_htmlbanners7_slides` as id_slide
                FROM `'._DB_PREFIX_.'htmlbanners7` hs
                WHERE hs.`id_htmlbanners7_slides` = '.(int)$id_slide;
    ...
    public function renderList()
    {
        ...
        foreach ($slides as $key => $slide) {
            ...
            $associated_shop_ids = HtmlBanner7::getAssociatedIdsShop((int)$slide['id_slide']);
