...
include_once('htmlbanners10.php');

$hero_slider = new HtmlBanners10();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides')) {
    ...
    foreach ($slides as $position => $id_slide) {
        $res = Db::getInstance()->execute(
            '
            UPDATE `'._DB_PREFIX_.'htmlbanners10_slides` SET `position` = '.(int)$position.'
            WHERE `id_htmlbanners10_slides` = '.(int)$id_slide
        );
    }
