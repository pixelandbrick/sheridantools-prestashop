...
if(isset($_POST) && !empty($_POST)){
    ...
    if(isset($_POST['name']) && !empty($_POST['name'])){
        ...
        $results['error'][] = Context::getContext()->getTranslator()->trans("Name Is Required", array(), 'Shop.Interiorblog');  
    }
    if(isset($_POST['email']) && !empty($_POST['email']) && Validate::isEmail($_POST['email'])){
        $commentobj->email = $_POST['email'];
    }else{
        $results['error'][] = Context::getContext()->getTranslator()->trans("Valid Email Address Is Required", array(), 'Shop.Interiorblog');   
    }
    $commentobj->website = (isset($_POST['website']) && !empty($_POST['website'])) ? $_POST['website'] : '#';
    if(isset($_POST['subject']) && !empty($_POST['subject'])){
        $commentobj->subject = $_POST['subject'];
    }else{
        $results['error'][] = Context::getContext()->getTranslator()->trans("Subject Is Required", array(), 'Shop.Interiorblog');
    }
    if(isset($_POST['content']) && !empty($_POST['content'])){
        $commentobj->content = $_POST['content'];
    }else{
        $results['error'][] = Context::getContext()->getTranslator()->trans("Comment Content Is Required", array(), 'Shop.Interiorblog');
    }
    ...
    if(isset($results['error']) && !empty($results['error'])){
        ...
        if($commentobj->add()){
            ...
            $results['success'][] = Context::getContext()->getTranslator()->trans("Successfully Comment Added", array(), 'Shop.Interiorblog');
            $results['results'] = $commentobj;
            die(Tools::jsonEncode($results));
        }else{
            $results['error'][] = Context::getContext()->getTranslator()->trans("Something Wrong Please Try Again ! ", array(), 'Shop.Interiorblog');
            die(Tools::jsonEncode($results));   
        }
    }
}