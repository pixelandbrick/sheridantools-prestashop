...
class RelatedProductProModel extends ObjectModel
{
    ...
    /**
     * Get custom settings - override global settings
     */
    public function getCustomSettings()
    {
        $sql = 'SELECT * FROM `'._DB_PREFIX_.'product_related_pro_custom`
     ...
     * Get custom settings - override global settings
     */
    public function saveCustomSettings($is_custom = 0, $id_type = 0)
    {
        $sql = 'SELECT * FROM `'._DB_PREFIX_.'product_related_pro_custom`
            WHERE `id_product` = '.(int)$this->id_product.' 
            AND `id_shop` = '.(int)$this->id_shop.'
        ...
        if (!empty($data)) {
            $sql = 'UPDATE `'._DB_PREFIX_.'product_related_pro_custom`
                SET is_custom = '.(int)$is_custom.',
                id_type = '.(int)$id_type.'
                WHERE
            ...
            $sql = 'INSERT INTO `'._DB_PREFIX_.'product_related_pro_custom` (id_product, id_shop, is_custom, id_type)';
            $sql .= ' VALUES (';
        ...
        if ($relation == 3) {
            return Db::getInstance()->executeS('
                    SELECT DISTINCT cp.id_product
                        ...
                        WHERE cp.id_product != '.(int)$this->id_product.'
                        AND p.active = 1
                        AND cp.id_category IN (
                                SELECT `id_category`
                                FROM `'._DB_PREFIX_.'category_product`
                                WHERE `id_product` = '.(int)$this->id_product.' ORDER BY `position` DESC
                        )
    ...
    {
        $sql = 'SELECT COUNT(*) FROM `'._DB_PREFIX_.'product_related_pro` WHERE
    ...
    public function relatedUpdate($related_product)
    {
        ...
        foreach ($related_product as $key => $id_related) {
            if ($id_related) {
                $model = new RelatedProductProModel($this->id_product, $this->id_shop);
    ...
    {
        $sql = 'INSERT INTO `'._DB_PREFIX_.'product_related_pro` (id_product, id_related, id_shop, position)';
        $sql .= ' VALUES (';
        $sql .= (int)$this->id_product.',';
    ...
    {
        $sql = 'UPDATE `'._DB_PREFIX_.'product_related_pro` SET position = '.(int)$this->position.'
    ...
    {
        $sql = 'DELETE FROM `'._DB_PREFIX_.'product_related_pro` WHERE';
    ...
    {
        $sql = 'SELECT * FROM `'._DB_PREFIX_.'product_related_pro`';
        return Db::getInstance()->executeS($sql);
    }
    
    public function getAllByRef()
    {
        $sql = 'SELECT p1.reference AS ref_product, p2.reference AS ref_related, prp.id_shop AS id_shop, prp.position AS position
        FROM `'._DB_PREFIX_.'product_related_pro` prp
        LEFT JOIN `'._DB_PREFIX_.'product` p1 ON (p1.`id_product` = prp.`id_product`)
        LEFT JOIN `'._DB_PREFIX_.'product` p2 ON (p2.`id_product` = prp.`id_related`)';
        return Db::getInstance()->executeS($sql);
    }

    public function getProductIdByReference($reference = null)
    {
