...
class AuthorizedotnetCIM
{
    ...
    public function saveProfile($data)
    {
        ...
        if (!$valsProfile) {
            ...
            if ($parsedProfileResponse->messages->resultCode == 'Ok') {
                ...
                Db::getInstance()->insert('authorizedotnet_cim_profile', $valsProfile);
            } else {
        ...
        if (!count($this->_errors)) {
            ...
            if ($parsedPaymentProfileResponse->messages->resultCode == 'Ok') {
                ...
                if (empty($data['customer_payment_profile_id'])) {
                    Db::getInstance()->insert('authorizedotnet_cim_payment_profile', $valsPaymentProfile);
                } else {
                    Db::getInstance()->update('authorizedotnet_cim_payment_profile', $valsPaymentProfile, 'customer_payment_profile_id = ' . (int) $data['customer_payment_profile_id']);
                }
            } else {
    ...
    public function sendRequestViaFsockopen($host, $path, $content)
    {
        ...
        if (!$fp) {
            ...
            $body = Tools::substr($response, $bodypos);
        }
        return $body;
    }
