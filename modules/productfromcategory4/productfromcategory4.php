...
class ProductFromCategory4 extends Module
{
    public function __construct()
    {
        $this->name = 'productfromcategory4';
        ...
        $this->displayName = $this->l('Products from selected category 4');
    ...
    {
        $this->_clearCache('*');
        Configuration::updateValue('PRODUCTFROMCATEGORY4_NBR', 8);
        Configuration::updateValue('PRODUCTFROMCATEGORY4_CAT', (int)Context::getContext()->shop->getCategory());
        Configuration::updateValue('PRODUCTFROMCATEGORY4_RANDOMIZE', false);
    ...
    public function fillMultilangValues()
    {
        ...
        foreach ($this->getMultilangFields(false) as $name => $data) {
            foreach ($languages as $lang) {
                $key = Tools::strtoupper('PRODUCTFROMCATEGORY4_'.$name).'_'.$lang['id_lang'];
    ...
    private function updateLanguageField($field_name)
    {
        ...
        $field = array($lang_default => Tools::getValue('productfromcategory4_'.$field_name.'_'.$lang_default));
        ...
        foreach ($this->context->controller->_languages as $lang) {
            ...
            $field_value = Tools::getValue('productfromcategory4_'.$field_name.'_'.(int)$lang['id_lang']);
        ...
        foreach ($this->context->controller->_languages as $lang) {
            Configuration::updateValue('PRODUCTFROMCATEGORY4_'.Tools::strtoupper($field_name).'_'.(int)$lang['id_lang'], $field[(int)$lang['id_lang']]);
        }
    }
    ...
    public function getContent()
    {
        $output = '';
        $errors = array();
        if (Tools::isSubmit('submitProductFromCategory4')) {
            $nbr = Tools::getValue('PRODUCTFROMCATEGORY4_NBR');
            if (!Validate::isInt($nbr) || $nbr <= 0) {
                $errors[] = $this->l('The number of products is invalid. Please enter a positive number.');
            }

            $cat = Tools::getValue('PRODUCTFROMCATEGORY4_CAT');
            if (!Validate::isInt($cat) || $cat <= 0) {
                $errors[] = $this->l('The category ID is invalid. Please choose an existing category ID.');
            }

            $rand = Tools::getValue('PRODUCTFROMCATEGORY4_RANDOMIZE');
            ...
            if (isset($errors) && count($errors)) {
                ...
                Configuration::updateValue('PRODUCTFROMCATEGORY4_NBR', (int)$nbr);
                Configuration::updateValue('PRODUCTFROMCATEGORY4_CAT', (int)$cat);
                Configuration::updateValue('PRODUCTFROMCATEGORY4_RANDOMIZE', (bool)$rand);
                Tools::clearCache(Context::getContext()->smarty, $this->getTemplatePath('productfromcategory4.tpl'));
    ...
    {
        $category = new Category((int)Configuration::get('PRODUCTFROMCATEGORY4_CAT'));
        $nProducts = Configuration::get('PRODUCTFROMCATEGORY4_NBR');

        if (Configuration::get('PRODUCTFROMCATEGORY4_RANDOMIZE') == 1) {
    ...
    public function _clearCache($template, $cache_id = null, $compile_id = null)
    {
        ...
        $cache_id;
        $compile_id;
        parent::_clearCache('productfromcategory4.tpl', 'productfromcategory4');
    }
    ...
    public function hookdisplayHomeTab($params)
    {
        ...
        foreach ($this->getMultilangFields() as $field_name) {
            $key = 'productfromcategory4_'.$field_name;
    ...
    public function hookdisplayHomeCustom($params)
    {
        ...
        $this->smarty->assign($multilang_array);
        return $this->display(__FILE__, 'productfromcategory4.tpl');
    }
    ...
    public function getAssignmentVariables()
    {
        return array(
            ...
            'allProductsLink' => Context::getContext()->link->getCategoryLink($this->getConfigFieldsValues()['PRODUCTFROMCATEGORY4_CAT']),
        );
    }
    ...
    public function renderForm()
    {
        ...
        foreach ($this->getMultilangFields(false) as $name => $data) {
            $multilang_fields[$name] = array(
                'type' => 'text',
                'label' => $data[0],
                'name' => 'productfromcategory4_'.$name,
                'lang' => true,
            );
        }
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'name' => 'PRODUCTFROMCATEGORY4_RANDOMIZE',
                        'class' => 'fixed-width-xs',
        ...
        $helper->submit_action = 'submitProductFromCategory4';
        ...
        foreach ($this->context->controller->_languages as $lang) {
            foreach ($this->getMultilangFields() as $field_name) {
                $field_name = 'productfromcategory4_'.$field_name;
    ...
    {
        return array(
            'PRODUCTFROMCATEGORY4_NBR' => Tools::getValue('PRODUCTFROMCATEGORY4_NBR', (int)Configuration::get('PRODUCTFROMCATEGORY4_NBR')),
            'PRODUCTFROMCATEGORY4_CAT' => Tools::getValue('PRODUCTFROMCATEGORY4_CAT', (int)Configuration::get('PRODUCTFROMCATEGORY4_CAT')),
            'PRODUCTFROMCATEGORY4_RANDOMIZE' => Tools::getValue('PRODUCTFROMCATEGORY4_RANDOMIZE', (bool)Configuration::get('PRODUCTFROMCATEGORY4_RANDOMIZE')),
        );
    }
}
