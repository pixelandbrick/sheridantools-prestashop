...
class ProductFromCategory1 extends Module
{
    public function __construct()
    {
        $this->name = 'productfromcategory1';
        ...
        $this->displayName = $this->l('Products from selected category 1');
    ...
    {
        $this->_clearCache('*');
        Configuration::updateValue('PRODUCTFROMCATEGORY1_NBR', 8);
        Configuration::updateValue('PRODUCTFROMCATEGORY1_CAT', (int)Context::getContext()->shop->getCategory());
        Configuration::updateValue('PRODUCTFROMCATEGORY1_RANDOMIZE', false);
    ...
    public function fillMultilangValues()
    {
        ...
        foreach ($this->getMultilangFields(false) as $name => $data) {
            foreach ($languages as $lang) {
                $key = Tools::strtoupper('PRODUCTFROMCATEGORY1_'.$name).'_'.$lang['id_lang'];
    ...
    private function updateLanguageField($field_name)
    {
        ...
        $field = array($lang_default => Tools::getValue('productfromcategory1_'.$field_name.'_'.$lang_default));
        ...
        foreach ($this->context->controller->_languages as $lang) {
            ...
            $field_value = Tools::getValue('productfromcategory1_'.$field_name.'_'.(int)$lang['id_lang']);
        ...
        foreach ($this->context->controller->_languages as $lang) {
            Configuration::updateValue('PRODUCTFROMCATEGORY1_'.Tools::strtoupper($field_name).'_'.(int)$lang['id_lang'], $field[(int)$lang['id_lang']]);
        }
    }
    ...
    public function getContent()
    {
        $output = '';
        $errors = array();
        if (Tools::isSubmit('submitProductFromCategory1')) {
            $nbr = Tools::getValue('PRODUCTFROMCATEGORY1_NBR');
            if (!Validate::isInt($nbr) || $nbr <= 0) {
                $errors[] = $this->l('The number of products is invalid. Please enter a positive number.');
            }

            $cat = Tools::getValue('PRODUCTFROMCATEGORY1_CAT');
            if (!Validate::isInt($cat) || $cat <= 0) {
                $errors[] = $this->l('The category ID is invalid. Please choose an existing category ID.');
            }

            $rand = Tools::getValue('PRODUCTFROMCATEGORY1_RANDOMIZE');
            ...
            if (isset($errors) && count($errors)) {
                ...
                Configuration::updateValue('PRODUCTFROMCATEGORY1_NBR', (int)$nbr);
                Configuration::updateValue('PRODUCTFROMCATEGORY1_CAT', (int)$cat);
                Configuration::updateValue('PRODUCTFROMCATEGORY1_RANDOMIZE', (bool)$rand);
                Tools::clearCache(Context::getContext()->smarty, $this->getTemplatePath('productfromcategory1.tpl'));
    ...
    {
        $category = new Category((int)Configuration::get('PRODUCTFROMCATEGORY1_CAT'));
        $nProducts = Configuration::get('PRODUCTFROMCATEGORY1_NBR');

        if (Configuration::get('PRODUCTFROMCATEGORY1_RANDOMIZE') == 1) {
    ...
    public function _clearCache($template, $cache_id = null, $compile_id = null)
    {
        ...
        $cache_id;
        $compile_id;
        parent::_clearCache('productfromcategory1.tpl', 'productfromcategory1');
    }
    ...
    public function hookdisplayHomeTab($params)
    {
        ...
        foreach ($this->getMultilangFields() as $field_name) {
            $key = 'productfromcategory1_'.$field_name;
    ...
    public function hookdisplayHomeCustom($params)
    {
        ...
        $this->smarty->assign($multilang_array);
        return $this->display(__FILE__, 'productfromcategory1.tpl');
    }
    ...
    public function getAssignmentVariables()
    {
        return array(
            ...
            'allProductsLink' => Context::getContext()->link->getCategoryLink($this->getConfigFieldsValues()['PRODUCTFROMCATEGORY1_CAT']),
        );
    }
    ...
    public function renderForm()
    {
        ...
        foreach ($this->getMultilangFields(false) as $name => $data) {
            $multilang_fields[$name] = array(
                'type' => 'text',
                'label' => $data[0],
                'name' => 'productfromcategory1_'.$name,
                'lang' => true,
            );
        }
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'name' => 'PRODUCTFROMCATEGORY1_RANDOMIZE',
                        'class' => 'fixed-width-xs',
        ...
        $helper->submit_action = 'submitProductFromCategory1';
        ...
        foreach ($this->context->controller->_languages as $lang) {
            foreach ($this->getMultilangFields() as $field_name) {
                $field_name = 'productfromcategory1_'.$field_name;
    ...
    {
        return array(
            'PRODUCTFROMCATEGORY1_NBR' => Tools::getValue('PRODUCTFROMCATEGORY1_NBR', (int)Configuration::get('PRODUCTFROMCATEGORY1_NBR')),
            'PRODUCTFROMCATEGORY1_CAT' => Tools::getValue('PRODUCTFROMCATEGORY1_CAT', (int)Configuration::get('PRODUCTFROMCATEGORY1_CAT')),
            'PRODUCTFROMCATEGORY1_RANDOMIZE' => Tools::getValue('PRODUCTFROMCATEGORY1_RANDOMIZE', (bool)Configuration::get('PRODUCTFROMCATEGORY1_RANDOMIZE')),
        );
    }
}
