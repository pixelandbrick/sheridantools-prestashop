...
class ProductFromCategory2 extends Module
{
    public function __construct()
    {
        $this->name = 'productfromcategory2';
        ...
        $this->displayName = $this->l('Products from selected category 2');
    ...
    {
        $this->_clearCache('*');
        Configuration::updateValue('PRODUCTFROMCATEGORY2_NBR', 8);
        Configuration::updateValue('PRODUCTFROMCATEGORY2_CAT', (int)Context::getContext()->shop->getCategory());
        Configuration::updateValue('PRODUCTFROMCATEGORY2_RANDOMIZE', false);
    ...
    public function fillMultilangValues()
    {
        ...
        foreach ($this->getMultilangFields(false) as $name => $data) {
            foreach ($languages as $lang) {
                $key = Tools::strtoupper('PRODUCTFROMCATEGORY2_'.$name).'_'.$lang['id_lang'];
    ...
    private function updateLanguageField($field_name)
    {
        ...
        $field = array($lang_default => Tools::getValue('productfromcategory2_'.$field_name.'_'.$lang_default));
        ...
        foreach ($this->context->controller->_languages as $lang) {
            ...
            $field_value = Tools::getValue('productfromcategory2_'.$field_name.'_'.(int)$lang['id_lang']);
        ...
        foreach ($this->context->controller->_languages as $lang) {
            Configuration::updateValue('PRODUCTFROMCATEGORY2_'.Tools::strtoupper($field_name).'_'.(int)$lang['id_lang'], $field[(int)$lang['id_lang']]);
        }
    }
    ...
    public function getContent()
    {
        $output = '';
        $errors = array();
        if (Tools::isSubmit('submitProductFromCategory2')) {
            $nbr = Tools::getValue('PRODUCTFROMCATEGORY2_NBR');
            if (!Validate::isInt($nbr) || $nbr <= 0) {
                $errors[] = $this->l('The number of products is invalid. Please enter a positive number.');
            }

            $cat = Tools::getValue('PRODUCTFROMCATEGORY2_CAT');
            if (!Validate::isInt($cat) || $cat <= 0) {
                $errors[] = $this->l('The category ID is invalid. Please choose an existing category ID.');
            }

            $rand = Tools::getValue('PRODUCTFROMCATEGORY2_RANDOMIZE');
            ...
            if (isset($errors) && count($errors)) {
                ...
                Configuration::updateValue('PRODUCTFROMCATEGORY2_NBR', (int)$nbr);
                Configuration::updateValue('PRODUCTFROMCATEGORY2_CAT', (int)$cat);
                Configuration::updateValue('PRODUCTFROMCATEGORY2_RANDOMIZE', (bool)$rand);
                Tools::clearCache(Context::getContext()->smarty, $this->getTemplatePath('productfromcategory2.tpl'));
    ...
    {
        $category = new Category((int)Configuration::get('PRODUCTFROMCATEGORY2_CAT'));
        $nProducts = Configuration::get('PRODUCTFROMCATEGORY2_NBR');

        if (Configuration::get('PRODUCTFROMCATEGORY2_RANDOMIZE') == 1) {
    ...
    public function _clearCache($template, $cache_id = null, $compile_id = null)
    {
        ...
        $cache_id;
        $compile_id;
        parent::_clearCache('productfromcategory2.tpl', 'productfromcategory2');
    }
    ...
    public function hookdisplayHomeTab($params)
    {
        ...
        foreach ($this->getMultilangFields() as $field_name) {
            $key = 'productfromcategory2_'.$field_name;
    ...
    public function hookdisplayHomeCustom($params)
    {
        ...
        $this->smarty->assign($multilang_array);
        return $this->display(__FILE__, 'productfromcategory2.tpl');
    }
    ...
    public function getAssignmentVariables()
    {
        return array(
            ...
            'allProductsLink' => Context::getContext()->link->getCategoryLink($this->getConfigFieldsValues()['PRODUCTFROMCATEGORY2_CAT']),
        );
    }
    ...
    public function renderForm()
    {
        ...
        foreach ($this->getMultilangFields(false) as $name => $data) {
            $multilang_fields[$name] = array(
                'type' => 'text',
                'label' => $data[0],
                'name' => 'productfromcategory2_'.$name,
                'lang' => true,
            );
        }
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'name' => 'PRODUCTFROMCATEGORY2_RANDOMIZE',
                        'class' => 'fixed-width-xs',
        ...
        $helper->submit_action = 'submitProductFromCategory2';
        ...
        foreach ($this->context->controller->_languages as $lang) {
            foreach ($this->getMultilangFields() as $field_name) {
                $field_name = 'productfromcategory2_'.$field_name;
    ...
    {
        return array(
            'PRODUCTFROMCATEGORY2_NBR' => Tools::getValue('PRODUCTFROMCATEGORY2_NBR', (int)Configuration::get('PRODUCTFROMCATEGORY2_NBR')),
            'PRODUCTFROMCATEGORY2_CAT' => Tools::getValue('PRODUCTFROMCATEGORY2_CAT', (int)Configuration::get('PRODUCTFROMCATEGORY2_CAT')),
            'PRODUCTFROMCATEGORY2_RANDOMIZE' => Tools::getValue('PRODUCTFROMCATEGORY2_RANDOMIZE', (bool)Configuration::get('PRODUCTFROMCATEGORY2_RANDOMIZE')),
        );
    }
}
