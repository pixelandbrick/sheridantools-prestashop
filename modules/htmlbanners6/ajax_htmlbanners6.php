...
include_once('htmlbanners6.php');

$hero_slider = new HtmlBanners6();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides')) {
    ...
    foreach ($slides as $position => $id_slide) {
        $res = Db::getInstance()->execute(
            '
			UPDATE `'._DB_PREFIX_.'htmlbanners6_slides` SET `position` = '.(int)$position.'
			WHERE `id_htmlbanners6_slides` = '.(int)$id_slide
        );
    }
