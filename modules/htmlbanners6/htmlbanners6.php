...
include_once(_PS_MODULE_DIR_.'htmlbanners6/HtmlBanner6.php');

class HtmlBanners6 extends Module implements WidgetInterface
{
    protected $_html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('Custom background image');
        $this->description = $this->l('Set custom background image by CSS selector');
        $this->ps_versions_compliancy = array('min' => '1.7.1.0', 'max' => _PS_VERSION_);

        $this->templateFile = 'module:htmlbanners6/views/templates/hook/hook.tpl';
    }
    ...
    protected function installSamples()
    {
        ...
        $slide = new HtmlBanner6();
        $slide->position = 1;
        $slide->active = 1;
        foreach ($languages as $language) {
            $slide->title[$language['id_lang']] = 'Testimonials background';
            $slide->customclass[$language['id_lang']] = '.testimonials-wrapper .testimonials-bg';
        ...
        $slide->active = 1;
        foreach ($languages as $language) {
            $slide->title[$language['id_lang']] = 'Background image of header on page exept homepage';
            $slide->customclass[$language['id_lang']] = '.is-not-index #header';
    ...
    protected function createTables()
    {
        ...
        $res = (bool)Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'htmlbanners6` (
                `id_htmlbanners6_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
                `id_shop` int(10) unsigned NOT NULL,
                PRIMARY KEY (`id_htmlbanners6_slides`, `id_shop`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');

        /* Slides configuration */
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'htmlbanners6_slides` (
              `id_htmlbanners6_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
              `position` int(10) unsigned NOT NULL DEFAULT \'0\',
              `active` tinyint(1) unsigned NOT NULL DEFAULT \'0\',
              PRIMARY KEY (`id_htmlbanners6_slides`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');

        /* Slides lang configuration */
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'htmlbanners6_slides_lang` (
              `id_htmlbanners6_slides` int(10) unsigned NOT NULL,
              `id_lang` int(10) unsigned NOT NULL,
              `title` varchar(255) NOT NULL,
              `customclass` varchar(255) NOT NULL,
              `image` varchar(255) NOT NULL,
              PRIMARY KEY (`id_htmlbanners6_slides`,`id_lang`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');
    ...
    protected function deleteTables()
    {
        ...
        foreach ($slides as $slide) {
            $to_del = new HtmlBanner6($slide['id_slide']);
            $to_del->delete();
        }

        return Db::getInstance()->execute('
            DROP TABLE IF EXISTS `'._DB_PREFIX_.'htmlbanners6`, `'._DB_PREFIX_.'htmlbanners6_slides`, `'._DB_PREFIX_.'htmlbanners6_slides_lang`;
        ');
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitSlide') || Tools::isSubmit('delete_id_slide') ||
            ...
            if ($mode == 'add') {
                ...
                $associated_shop_ids = HtmlBanner6::getAssociatedIdsShop((int)Tools::getValue('id_slide'));
    ...
    protected function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            $slide = new HtmlBanner6((int)Tools::getValue('id_slide'));
            if ($slide->active == 0) {
                $slide->active = 1;
            } else {
                $slide->active = 0;
            ...
            $slide = new HtmlBanner6((int)Tools::getValue('delete_id_slide'));
            $res = $slide->delete();
            $this->clearCache();
            if (!$res) {
    ...
    public function hookActionShopDataDuplication($params)
    {
        Db::getInstance()->execute(
            '
            INSERT IGNORE INTO '._DB_PREFIX_.'htmlbanners6 (id_htmlbanners6_slides, id_shop)
            SELECT id_htmlbanners6_slides, '.(int)$params['new_id_shop'].'
            FROM '._DB_PREFIX_.'htmlbanners6
    ...
    public function getNextPosition()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow(
            ...
            FROM `'._DB_PREFIX_.'htmlbanners6_slides` hss, `'._DB_PREFIX_.'htmlbanners6` hs
            WHERE hss.`id_htmlbanners6_slides` = hs.`id_htmlbanners6_slides` AND hs.`id_shop` = '.(int)$this->context->shop->id
        );
    ...
    public function getSlides($active = null)
    {
        ...
        $slides = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            '
            SELECT hs.`id_htmlbanners6_slides` as id_slide, hss.`position`, hss.`active`, hssl.`title`,
            hssl.`customclass`, hssl.`image`
            FROM '._DB_PREFIX_.'htmlbanners6 hs
            LEFT JOIN '._DB_PREFIX_.'htmlbanners6_slides hss ON (hs.id_htmlbanners6_slides = hss.id_htmlbanners6_slides)
            LEFT JOIN '._DB_PREFIX_.'htmlbanners6_slides_lang hssl ON (hss.id_htmlbanners6_slides = hssl.id_htmlbanners6_slides)
        ...
        foreach ($slides as &$slide) {
            $slide['image_url'] = $this->context->link->getMediaLink(_MODULE_DIR_.'htmlbanners6/views/img/upload/'.$slide['image']);
        }
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            ...
            LEFT JOIN '._DB_PREFIX_.'htmlbanners6_slides hss ON (hs.id_htmlbanners6_slides = hss.id_htmlbanners6_slides)
            LEFT JOIN '._DB_PREFIX_.'htmlbanners6_slides_lang hssl ON (hss.id_htmlbanners6_slides = hssl.id_htmlbanners6_slides)
            WHERE hs.`id_htmlbanners6_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
            ($active ? ' AND hss.`active` = 1' : ' ')
        );
    ...
    {
        $req = 'SELECT hs.`id_htmlbanners6_slides` as id_slide
                FROM `'._DB_PREFIX_.'htmlbanners6` hs
                WHERE hs.`id_htmlbanners6_slides` = '.(int)$id_slide;
    ...
    public function renderList()
    {
        ...
        foreach ($slides as $key => $slide) {
            ...
            $associated_shop_ids = HtmlBanner6::getAssociatedIdsShop((int)$slide['id_slide']);
